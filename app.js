...
var addSongToLibrary = function(title, notes) {
  $('#library-list').append("<li class='noselect'>" +
...
$(document).ready(function() {
  ...
  // click on trash icon removes song
  $("li .fa-trash").on("click", function(){
    $(this).parent().slideUp(500, function(){
      $(this).remove(); // the "this" here is the "this.parent" from above
    });  
  });

  //  dbl click on song title slideDown to reveal notes
  $("ul").on("dblclick", "li", function(){
    $(this).find(".notes").slideToggle(300);
  });

  // message fade in and then out
  $("#message").fadeIn(800, function(){
    setTimeout(function() { $("#message").fadeOut(800); }, 3000);
  });

  $( "#playlist-list, #library-list" ).sortable({connectWith: ".connectedSortable"});
  $( "#playlist-list, #library-list" ).disableSelection();

  // filter the library
  $('#filter-library').on('keyup', function () {
    var text1 = $(this).val();
    $( "li" ).hide();
    $( "li" ).filter(':contains(' + text1 + ')').show();
  });
